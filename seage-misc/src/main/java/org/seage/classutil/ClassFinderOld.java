...
public class ClassFinderOld {
  ...
  private static File[] searchForJars(String rootDir, String jarPrefix) throws Exception {
    ...
    // System.out.println(loader.getResource(this.getClass().getCanonicalName().replace(".",
    // "/")+".class"));
  ...
  private static URLClassLoader createClassLoader(JarFile jarFile, String jarPath) throws Exception {
    ...
    // urls[1] = new
    // URL("file:///mirror/rick/Projects/seage/seage.problems/discrete/qap/dist/lib/seage.metaheuristics.jar");

    // for(int i = 0;i< urls.length;i++)
    // if(paths[i].charAt(0) != '/' && paths[i].charAt(1) != ':')
    //// urls[i+1] = new URL("file://" +new File(jarPath).getParent() + "/" +
    // paths[i]);
    // method.invoke(result, new Object[]{new URL("file://" +new
    // File(jarPath).getParent() + "/" + paths[i])});
  ...
  // private static ClassInfo createClassInfo(JarFile jar, String jarPath, Class
