...
public class LeftParse {
  ...
  public NonterminalSymbol createParseTree(Grammar gr) throws Exception {
    ...
    /*
     * NOTE: how many items may be on stack to reset recursion count for given rule
     * set
     */
    ...
    while (!stack.empty()) {
      ...
      if (rec == null) {
        ...
        // set current count on stack to release recursion (used grammar rule is done by
        // that count)
