...
public class Grammar {
  ...
  public void calculateTerminateRules() throws Exception {
    /**
     * ALGORITHM: iterate through grammar and test if all symbols on right side of
     * grammar are terminals or members of "terminatable", if so add the rule to
     * terminating, and symbol on left side to "terminatable". iterate as long as
     * the set grows
     */
    ...
    while (added) {
      ...
      while (it.hasNext()) {
        ...
        while (rit.hasNext()) {
          Symbol s = rit.next();
          if (s.getType() != Symbol.Type.TERMINAL && !terminatable.contains(s) && !addTerminatable.contains(s))
