...
public class TspGeneticOperator extends GeneticOperator
{
	...
	public Subject[] crossOver2(Subject parent1, Subject parent2) throws Exception
	{
		...
		for (int i = 0; i < length; i++)
		{
			if (i < ix || i >= ix + crossLength)
			{
				...
				while (c1 != -1)
				{
					c1 = cross1[(Integer) ch1.getGene(i).getValue()];
					if (c1 != -1)
					{
				...
				while (c2 != -1)
				{
					c2 = cross2[(Integer) ch2.getGene(i).getValue()];
					if (c2 != -1)
					{
			...
			occur1[(Integer) ch1.getGene(i).getValue()]++;
			occur2[(Integer) ch2.getGene(i).getValue()]++;
		}
	...
	public Subject[] crossOver(Subject parent1, Subject parent2) throws Exception
	{
		...
		for (int i = 0; i < length; i++)
		{
			chromVals1.put(i, (Integer) child1Chrom.getGene(i).getValue());
			chromVals2.put(i, (Integer) child2Chrom.getGene(i).getValue());
		}

		for (int i = ix; i < ix + crossLength; i++)
		{

			int x1 = (Integer) child1Chrom.getGene(i).getValue();
			int x2 = (Integer) child2Chrom.getGene(i).getValue();
