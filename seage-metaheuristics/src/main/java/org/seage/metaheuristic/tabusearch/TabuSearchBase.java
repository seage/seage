...
public abstract class TabuSearchBase implements ITabuSearch {
  ...
  /**
   * Tracks the number of iterations comleted since the inception of this tabu
   * search.
   *
   * @since 1.0a
   */
   ...
   * called by all implementations of {@link TabuSearchBase} at the end of an
   * iteration.
   *
   * @since 1.0a
   */
  ...
  /**
   * An array of {@link TabuSearchListener}s. An array is used instead of an
   * {@link javax.swing.event.EventListenerList} or a {@link java.util.Vector}.
   * This reduces the tabu search package's dependence on java's objects. Also
   ...
   */
  @Override
  public final synchronized void removeTabuSearchListener(TabuSearchListener listener) { // Find location of listener to
                                                                                         // remove
    int index = -1;
    int j = 0;
