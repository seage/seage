...
public abstract class FireflyOperator {
  ...
  /**
   * Attracts the solution s0 to solution s1 by formula: s0 = s0 + beta*s1 +
   * r*(rand - 0.5) where beta can be beta =
   * para.initialIntensity*exp(-absorption*distance^2) or beta =
   * para.initialIntensity/(1+absorption*distance^2) for faster computation and r
   * = para.initialRandomness if para.withDecreasingRandomness==false otherwise r
   * = decreaseRandomness(para, iter)
   * 
