...
public class ContinuousGeneticOperator<S extends Subject<Double>> extends BasicGeneticOperator<S, Double> // private
                                                                                                          // double
                                                                                                          // _crossLengthPct;
{
    ...
    for (Subject<Double> s : children) {
      for (int i = 0; i < parent1.getChromosome().getLength(); i++) {
        if (_crossLengthCoef > _random.nextDouble()) {
          double value = (parent1.getChromosome().getGene(i) + parent2.getChromosome().getGene(i)) / 2;
